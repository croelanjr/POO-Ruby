...
class Empleado
	...
	def self.promedio_salario
		promedio = 0.0
		total_promedio = 0.0
		@postulantes.each do |postula|
			promedio = promedio + postula.salario
			total_promedio = promedio / @postulantes.size
		end
		puts "El promedio Salario a pagar para los postulantes ganados es S/. #{total_promedio.round(2)} Nuevos Soles"
	end

	def self.cantidad_dinero
		puntos = 0
		cantidad = 0
		promedio = 0.0
		total_promedio = 0.0
		...
		puts "La cantidad de Dinero que necesita pagar a los Postulantes Ganadores es S/. #{(total_promedio*cantidad).round(2)} Nuevos Soles"
	end
